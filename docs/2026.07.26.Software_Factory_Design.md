...
graph TD
    ...
    PLAN_OUT -->|"+sf:plan-review (parks)"| HUMAN["👤 Human reviews plan comment<br/>adds sf:plan-approved"]
...
| `PLAN` (`/sf-plan`) | **`claude -p`** | Headless session generates plan; parks at `sf:plan-review` (approval is **label-driven**, no live session) | Fable 5 |
...
**Plan *generation* is autonomous** — the dispatcher launches `/sf-plan` automatically once `sf:tickets` is set, exactly like stages 1–2. What stays human is the **approval gate**, which is **label-driven** (not an interactive tmux block — a headless `claude -p` run cannot block for input): the agent generates the plan headlessly, `sf-apply-plan.sh` posts it and adds `sf:plan-review`, and the issue **parks** there. A human reviews the plan comment and approves by adding `sf:plan-approved` (via `sf-approve-plan.sh <N>` or the GitHub UI) — that label is what unblocks stage 3b. This preserves the "design before code" judgment gate while removing the manual step of kicking off planning. A good plan gates everything downstream, so this stage uses **Fable 5**.

| Aspect | Details |
|--------|---------|
| **Executor** | *Generation:* `claude -p /sf-plan <N>` (headless, dispatcher-launched) + `sf-apply-plan.sh` (trusted, → `sf:plan-review`). *Approval:* `sf-approve-plan.sh <N>` (human-only, → `sf:plan-approved`) |
...
| **Human Input** | (Later: notified via Slack.) Reviews the plan comment, then adds `sf:plan-approved` to approve — or edits tickets/re-runs if the plan is wrong |
| **Approval Flow** | Issue parks at `sf:plan-review`; the dispatcher never advances past it. Human adds `sf:plan-approved` → stage 3b becomes eligible. One human decision, label-driven — no live session to attach to |
| **Time Estimate** | ~1 minute (agent, autonomous) + human review time |
| **Next Step** | Approved → label `sf:plan-approved`; stage 3b creates `wt-impl-<N>` and begins |
...
| Aspect | Details |
|--------|---------|
| **Executor** | *Prep:* `sf-prep-worktree.sh <N>` (isolated `wt-impl-N`, branch `sf/impl-N`). *Code:* `claude -p /sf-dev <N>` headless, cwd = worktree (commits locally). *Publish:* `sf-apply-dev.sh` (trusted: push + draft PR + `sf:implemented`) |
| **Model** | Opus — code generation quality |
| **Trigger** | **Automatic:** label `sf:plan-approved` (dispatcher-launched) |
...
| **Next Step** | Label `sf:implemented` (set at PR open); PR merge → stage 4 (test) begins |
...
| **Trigger** | Label `sf:implemented` (dispatcher-launched) |
| **Config** | `.sf.yml` `test:` — the shell command to run (e.g. `bash tests/smoke.sh`, or `cd sub && pytest -q` for a monorepo). No command → test skipped, issue left at `sf:implemented` with a note |
| **Worktree** | Reuses/creates `~/sf-worktrees/<repo>/impl-<N>` on branch `sf/impl-<N>` |
| **Steps** | <ol><li>Guard: require `sf:implemented`, skip if already tested</li><li>Check out `sf/impl-N`</li><li>Read `.sf.yml` `test:`</li><li>Run it, capture exit code + output</li></ol> |
| **On Pass** (exit 0) | Label `sf:ready-for-prod`; comment "✅ tests pass" with output (collapsed) → stage 5 awaits explicit human go |
| **On Fail** (exit ≠0) | Label `sf:needs-debug`; comment the failing output tail |
...
| **Trigger** | **Human** runs `sf-prod.sh <N>` on `sf:ready-for-prod`. Never dispatcher-triggered (see Risks) |
| **Config** | `.sf.yml` `deploy:` — the shell command to deploy (run in the issue's worktree) |
| **Worktree** | Reuses/creates `sf/impl-<N>` worktree |
| **On Success** (exit 0) | Label `sf:deployed`; post 🚀 "Shipped" + the LLM deploy report; **close the issue** |
| **On Failure** (exit ≠0) | Label `sf:deploy-failed`; post the report + deploy-log tail; issue stays open for post-mortem |
...
│   ├── sf-apply-plan.sh     #   ✅ +sf:plan-review (parks at approval gate)
│   ├── sf-approve-plan.sh   #   ✅ human approval → +sf:plan-approved
│   ├── sf-prep-worktree.sh  #   ✅ isolated wt-impl-N (branch sf/impl-N)
│   ├── sf-apply-dev.sh      #   ✅ push + draft PR → +sf:implemented
...
│   ├── sf-install.sh        #   ✅ one-command onboarding (prereqs+labels+.sf.yml+report)
...
| 3a plan | `commands/sf-plan.md` | `scripts/sf-apply-plan.sh` (gen → `sf:plan-review`) + `scripts/sf-approve-plan.sh` (human → `sf:plan-approved`) | Fable 5 | `sf:tickets` → **+**`sf:plan-review` → *(human)* **+**`sf:plan-approved` | ✅ Done |
| 3b dev | `commands/sf-dev.md` | `scripts/sf-prep-worktree.sh` (worktree) + `scripts/sf-apply-dev.sh` (push + draft PR) | Opus | `sf:plan-approved` → **+**`sf:implemented` (draft PR; human reviews before merge) | ✅ Done |
| 4 test | *(none — script-only gate)* | `scripts/sf-test.sh` (runs `.sf.yml` `test:`, gates on exit code) | none | `sf:implemented` → **+**`sf:ready-for-prod` \| **+**`sf:needs-debug` | ✅ Done |
| 5 prod | `commands/sf-prod.md` (summary only) | `scripts/sf-prod.sh` (**human-run**; runs `.sf.yml` `deploy:`, gates on exit code) | Sonnet (summary) | `sf:ready-for-prod` → **+**`sf:deployed` (closed) \| **+**`sf:deploy-failed` | ✅ Done |
| — | *(dispatcher)* | `scripts/sf-dispatcher.sh` | — | launches stages 0–4 on their trigger labels; parks at `sf:plan-review` (human approves) and at `sf:ready-for-prod` (human triggers stage 5) | ✅ Done |
...
| **Repo-agnostic skills** | Skills run **no hardcoded repo** — every `gh` call targets the current working directory's repo. The dispatcher `cd`s into the target checkout; scripts default `REPO` to that dir's repo (override with `SF_REPO`). One plugin install (user scope) serves every repo on the machine — verified running triage on `databeacon/localr5` under the `david4aero` identity |
| **Onboarding a new repo** | ✅ **`/sf-install`** (or `bash scripts/sf-install.sh` from inside the repo): idempotent — prereq checks (git, `gh` auth + identity mismatch warning, `jq`, `tmux`), bootstraps the label state machine (`sf-init-labels.sh`), scaffolds a `.sf.yml` template if absent (never overwrites), prints a compliance report + human next steps. `sf-init-labels.sh` remains the labels-only primitive |
...
**Channel is per-project** (decision 2026.07.28): each repo sets `slack_channel:` in its `.sf.yml`; the `#software-factory` below is just this repo's example. The Slack **token is shared** (one workspace, fetched from Vaultwarden) — only the channel varies per repo. *(Notifications not yet built.)*
...
| Plan generated | "📋 Plan ready for approval (@david)" | #software-factory | Human action required; link to plan comment → approve by adding `sf:plan-approved` (`sf-approve-plan.sh <N>`) |
...
| `sf-plan-N` | Stage 3a agent (generation only) | Until plan generated | No (parks at `sf:plan-review`; approval is label-driven) |
...
2. **Approval flow:** ~~After `/sf-plan` generates a plan, should approval be manual (human attaches tmux) or via Slack reaction?~~ Resolved: **label-driven** — plan generation is headless, the issue parks at `sf:plan-review`, and a human approves by adding `sf:plan-approved` (`sf-approve-plan.sh`). A Slack reaction that adds the label can layer on later.
...
11. **Slack channel scoping:** ~~One shared channel or per-project?~~ Resolved (2026.07.28): **per-project** — each repo declares `slack_channel:` in its `.sf.yml`; the workspace token is shared (Vaultwarden), only the channel varies. Notification code (unbuilt) reads it from `.sf.yml`.
12. **Dispatcher timer scoping:** ~~One timer for all repos or per-project?~~ Resolved (2026.07.28): **per-project timers** via a systemd **template unit** `softwarefactory-dispatcher@<repo>.service` (`%i` = repo slug). Rationale: the deciding factor is **multi-account identity** — repos span `kilo9alfa` and `databeacon` (david4aero), and `gh`'s active account is machine-global and drifts; a single iterating dispatcher would have to `gh auth switch` per repo. A per-project unit instead **pins each repo to its account** via `Environment=GH_TOKEN=…` (from Vaultwarden, never hardcoded). Add `RandomizedDelaySec` so N repos don't fire simultaneously and spike Claude usage; `/sf-install` can optionally install+enable the instance. **Open caveat:** total concurrent Claude sessions across repos still needs a global cap (a limit on live `sf-*` tmux sessions) — a follow-up, not blocking. *(Not yet built — current dispatcher is single-repo via `SF_REPO_DIR`.)*
