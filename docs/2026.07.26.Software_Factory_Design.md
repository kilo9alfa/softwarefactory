...
| Aspect | Design |
|--------|--------|
| **Agent runtime** | Every stage is a Claude Code session: autonomous stages run headless (`claude --dangerously-skip-permissions -p "/sf-<stage> <N>"` inside tmux) — this now includes **stage 3a plan *generation*** (headless; it parks at the `sf:plan-review` label, no live session). Only **stage 3b dev** runs interactive `claude` in tmux, also with `--dangerously-skip-permissions`. The human gate is the *plan approval* (a label, `sf:plan-approved`), not per-tool prompts or a tmux attach |
...
│   ├── sf-dispatcher.sh     #   ✅ multi-stage poller (triage, spec, tickets, plan, dev)
...
| — | *(dispatcher)* | `scripts/sf-dispatcher.sh` | — | launches stages 0–3b on their trigger labels; parks at `sf:plan-review` until a human approves | ✅ Done |
...
| **Onboarding a new repo** | Today: `sf-init-labels.sh [owner/repo]` bootstraps the label state machine (idempotent). Later: a `/sf-install` command wraps labels + `.sf.yml` scaffold + auth/identity checks + validation into one step, once the full compliance list is settled (stages 4–5) |
