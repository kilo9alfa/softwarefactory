...
| **Per-project config** | The plugin is generic; each target repo opts in with a small `.sf.yml` at its root. Consumed today: `test:` (stage 4 gate command). Planned: `staging_url`, `deploy:` (stage 5), Slack channel. `sf-test.sh` reads it from the issue's branch, so config is versioned with the code |
