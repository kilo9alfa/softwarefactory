...
| **Onboarding a new repo** | ✅ **`/sf-install`** (or `bash scripts/sf-install.sh`): idempotent — prereq checks (git, `gh` auth + identity mismatch warning, `jq`, `tmux`), bootstraps labels (`sf-init-labels.sh`), scaffolds `.sf.yml` if absent, prints a compliance report. **`--enable-timer`** additionally writes `~/.config/softwarefactory/<slug>.env` and (on a systemd host) installs+enables the per-project dispatcher timer |
